 ...
 * 
 * @authors David Lowe <david.lowe@sydney.edu.au> + Joshua Wright <jwri9726@uni.sydney.edu.au>
 * @date 11/08/2015
 **/
...
{ 
		...
		this.controller = "SimpleCounterRig";
};
...
SimpleCounterControl.prototype.setup = function() {
        ...
        this.widgets.push(new DataValWidget(this.$container, 'Counter', 'info', 'Count=', 'counter'));
        this.widgets.push(new DataValWidget(this.$container, 'Increment', 'info', 'Inc=', 'increment'));
        // this.widgets.push(new DataValWidget(this.$container, 'Isst', 'info', '1=', 'sensor1'));
        // this.widgets.push(new DataValWidget(this.$container, 'Isst2', 'info', '2=', 'sensor2'));
        // this.widgets.push(new DataValWidget(this.$container, 'Isst3', 'info', '3=', 'sensor3'));
        // this.widgets.push(new DataValWidget(this.$container, 'Isst4', 'info', '4=', 'sensor6'));
        // this.widgets.push(new DataValWidget(this.$container, 'Isst5', 'info', '5=', 'sensor10'));
        this.widgets.push(new DataValWidgetSensor(this.$container, 'Sensors', 'info', 'sensor'));
		this.widgets.push(new DataValWidgetTwo(this.$container, 'test', 'info', 'count=', 'inc=','counter','increment'));
		// this.widgets.push(new ButtonWidget(this.$container, 'button','Increment','setCounterInc'));
		
		// graph1 = new GraphWidget(this.$container, "graphhdh");
		// graph1.setDataVariable('counter','Counter','#00ff00',-10,100);
		// graph1.setAxisLabels('time','counter');
		//graph1.isPulling = true;
		// this.widgets.push(graph1);
		...
		countincwidget = new SliderWidget(this.$container, 'Incrementing', 'increment', 'increment', 'setCounterInc');
		countincwidget.setRange(-10,10);
		countincwidget.setOrientation(false);
		countincwidget.setLabels('Inc', '');
		this.widgets.push(countincwidget); 
		
		heaterTempCont = new SliderWidget(this.$container, 'Heater Temp', 'heaterTemp', 'heaterTemp', 'setHeaterTemp');
		heaterTempCont.setRange(40, 200);
		heaterTempCont.setOrientation(false);
		heaterTempCont.setLabels('Heater Temp','Deg.C');
		this.widgets.push(heaterTempCont);
...
SimpleCounterControl.prototype.acquireLoop = function() {
        var thiz = this;
        $.ajax({
                url: "/primitive/mapjson/pc/SimpleCounterRigController/pa/getCounterVals",
                data: {
...
Widget.prototype.postControl = function(action, params, responseCallback, errorCallback) {
    var thiz = this;
	$.ajax({
        url: "/primitive/mapjson/pc/SimpleCounterRigController/pa/" + action,
        data: params,
        success: function(data) {
...
CameraWidget.prototype.consume = function(data) {
   ...
   var temp;
	...
	var hexstring = eval(uni);
 	for(var i = 1; i <= 11; i++){
		ValToShow =( (data["sensor"+(i-1)])).toPrecision(3);
	...
	/* SMALL ARROW SHOWING GRADIENT COMPARED TO LAST ONLY*/
	 	...
	 	for(var i = 0; i < 11; i++){
		ValToShow =( (data["sensor"+(i)])).toPrecision(3);
		if (i==0){
			ValToCompare= ( (data["heaterTemp"])).toPrecision(3);
		}
		else{
			ValToCompare= ( (data["sensor"+(i-1)])).toPrecision(3);
		}
	...
	ctx.beginPath();
	ctx.moveTo(0,this.videoHeight*(200-data["heaterTemp"])/160);
	temp=(200-data["heaterTemp"])/160;
	for(var i = 0; i < 11; i++){
		 ValToShow =( (data["sensor"+(i)])).toPrecision(3);	
 ...
 * Information displaying widget .
 */
function DataValWidgetSensor($container, title, icon, dataVar)
{
    Widget.call(this, $container, title, icon);
