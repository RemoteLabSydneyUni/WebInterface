...
{ 
		...
		this.controller = "SimpleCounterDLRig";
};
...
SimpleCounterControl.prototype.setup = function() {
        ...
        this.widgets.push(new SensorValWidget(this.$container, 'TSensors', 'info', 'Temperatures', 'Sensor-s'));

		/* create a widget for the temperature sensors */
		this.widgets.push(new PlaceHolderWidget(this.$container, 'Sensors', 'info', 'A message', 'info'));
		...
		/* Add widget to set arduino flash rate */
		flashratewidget = new SliderWidget(this.$container, 'FlashRate', 'flashperiod', 'flashperiod', 'setArduinoFlashPeriod');
		flashratewidget.setRange(0,10);
		flashratewidget.setOrientation(false);
		flashratewidget.setLabels('Period', '');
		this.widgets.push(flashratewidget); 
...
SimpleCounterControl.prototype.acquireLoop = function() {
        var thiz = this;
        $.ajax({
                url: "/primitive/mapjson/pc/SimpleCounterDLRigController/pa/getCounterVals",
                data: {
...
SimpleCounterControl.prototype.processData = function(data) {
		...
		if (parseFloat(data['counter'])>10) {
			$.ajax({
				url : "/primitive/mapjson/pc/SimpleCounterDLRigController/pa/setCounterInc",
				data : {
					increment: -0.1,
				},
				success: function(data){
...
Widget.prototype.postControl = function(action, params, responseCallback, errorCallback) {
    var thiz = this;
	$.ajax({
        url: "/primitive/mapjson/pc/SimpleCounterDLRigController/pa/" + action,
        data: params,
        success: function(data) {
