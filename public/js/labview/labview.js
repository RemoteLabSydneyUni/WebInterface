...
LabVIEWControl.prototype.setup = function() {
		...
		this.widgets.push(new ButtonWidget(this.$container, 'Add', "drain", "add", ""));
 
		this.widgets.push(new PlaceHolderWidget(this.$container, "Add Result", "drain", "Awaiting Result", 'res'));
 ...
 */
function SliderWidget($container, title, icon, dataVar, postAction, maxInput, minInput) 
{
...
SliderWidget.prototype.getHTML = function() {
    ...
    for (i = 0; i <= SliderWidget.NUM_SCALES; i++)
    {
        html += "<div class='slider-scale' style='" + (this.isVertical ? "top" : "left") + ":" + 
                    ...
                    "<span class='slider-scale-value'>" + (this.isVertical ? this.max - s * i : (s * i)+this.min) + "</span>" +
                "</div>";
    }
    html += "</div>";
