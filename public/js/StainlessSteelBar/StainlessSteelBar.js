/**
 * Stainless Steel Bar Web Interface.
 * 
 * @author Duncan Moss <duncan.moss@sydney.edu.au>
 * @date 22/2/2014
 **/
...
LabVIEWControl.prototype.setup = function() {
        /* Add camera to page. */
        this.widgets.push(new CameraWidget(this.$container, 'Camera', 'http://129.78.10.50/videostream.cgi?user=admin&pwd=passwd&resolution=32&rate=0', ''));
		
		/* Add Heater Control */
		heater = new SliderWidget(this.$container, "Heater Temperature", "heatertemp", "delay", "setHeater") 
		heater.setRange(30, 40);
		heater.setOrientation(false);
		heater.setLabels("Temp", "C");
		this.widgets.push(heater);
		
		/* Add Cooler Control */
		cooler = new SliderWidget(this.$container, "Cooler Temperature", "coolertemp", "delay", "setCooler") 
		cooler.setRange(15, 25);
		cooler.setOrientation(false);
		cooler.setLabels("Temp", "C");
		this.widgets.push(cooler);
        
		/* Stop Button */
		//this.widgets.push(new ButtonWidget(this.$container, 'Stop', "drain", "endLab", ""));
		
		/* Temp Graph */
		o = new GraphWidget(this.$container, "Bar Temperature");
		o.setDataVariable('pos-1', 'Temperature',  '#fcff00', 15, 45);
		o.setAxisLabels('Position', 'Temperature (C)');
		o.isPulling = true;
		this.widgets.push(o);
...
GraphWidget.prototype.getHTML = function() {
        ...
        {
                html += "<div class='graph-bottom-scale-" + i + "' style='left:" + (styleScale * i - 5) + "px'>" + i +"</div>";
        }
        html += "</div>";
...
GraphWidget.prototype.acquireData = function() {
        var thiz = this;
        $.ajax({
                url: "/primitive/mapjson/pc/LabVIEWController/pa/getTemp",
                data: {
                        period: this.period,
                        duration: this.duration,
                        from: 0,     // For now we are just asked for the latest data
...
GraphWidget.prototype.updateData = function(data) {
        var i = 0;
		tmpData = $.parseJSON(JSON.stringify(data)).res.split("Numeric").slice(1,12);
       if (this.startTime == undefined) this.startTime = data.start;
        this.latestTime = data.time;
        for (i in this.dataFields)
        {
                this.dataFields[i].values = tmpData;
                this.dataFields[i].seconds = 9000;
...
GraphWidget.prototype.drawTrace = function(dObj) {
        ...
        var xStep  = this.width / (dObj.seconds / this.period), 
...
SliderWidget.prototype.getHTML = function() {
    var i, s = (((this.max - this.min) / SliderWidget.NUM_SCALES)).toFixed(2),
        html = 
    ...
    {
		var value = (this.isVertical ? this.max - s * i : (s * i)+this.min)
        ...
        html += "<div class='slider-scale' style='" + (this.isVertical ? "top" : "left") + ":" + 
                    ...
                    "<span class='slider-scale-value'>" + value + "</span>" +
                "</div>";
    }
    html += "</div>";
