/**
 * Bar Temperature Web Interface.
 * 
 * @authors Yirui Deng <yden2600@uni.sydney.edu.au>
 * @date 20/07/2016
 **/

/* ============================================================================
 * == BarTemperatureControl.                                                     ==
 * ============================================================================ */
...
function BarTemperatureControl(id) 
{ 
        /** Display Manager. */
		...
		this.controller = "BarTemperatureRig";
 ...
 */
BarTemperatureControl.prototype.setup = function() {
        /* Add camera to page. */
		camera = new CameraWidget(this.$container, 'Camera', 'http://10.66.31.233/videostream.cgi?user=admin&pwd=passwd&resolution=32&rate=0', '');
        this.widgets.push(camera);
		/* Add widget for showing counter value*/
		this.widgets.push(new DataValWidgetSensor(this.$container,'Sensor','info','CurTemp','Sensor'));
		this.widgets.push(new RigStatusWidget(this.$container,'Mode','info','Type: ','RigInfo','Current Status: ','Mode'));
		this.widgets.push(new PidCtrlWidget(this.$container,'HeaterCtrl','info','Bar Heater End Temperature','Heater','Heater'));
		this.widgets.push(new PidCtrlWidget(this.$container,'CoolerCtrl','info','Bar Cooler End Temperature','Cooler','Cooler'));
		this.widgets.push(new PidTuneWidget(this.$container,'HeaterTuning','info','Bar Heater End Temperature','Heater','Heater'));
		this.widgets.push(new PidTuneWidget(this.$container,'CoolerTuning','info','Bar Cooler End Temperature','Cooler','Cooler'));
 ...
 * Runs the interface. 
 */
BarTemperatureControl.prototype.run = function() {
        /* Render the page. */
        this.display.init();
		/* Get Canvas context */

		this.ctx = document.getElementById('videoDrawingCanvas').getContext('2d');
		this.ctx.save();
		this.ctx.restore();
...
BarTemperatureControl.prototype.acquireLoop = function() {
        var thiz = this;
        $.ajax({
                url: "/primitive/mapjson/pc/BarTemperatureRigController/pa/getVals",
                data: {
 ...
 */
BarTemperatureControl.prototype.processData = function(data) {
		...
		if (parseFloat(data['counter'])>10) {
			$.ajax({
				url : "/primitive/mapjson/pc/SimpleCounterRigController/pa/setCounterInc",
				data : {
					increment: -0.1,
				},
				success: function(data){
 ...
 */
BarTemperatureControl.prototype.errorData = function(msg) {    
        if (!this.dataError)
        {
 ...
 */
function RigStatusWidget($container, title, icon, message1, dataVar1, message2, dataVar2)
{
    Widget.call(this, $container, title, icon);
    
    /** Identifier of this widget. */
    this.id = "DataVal-" + title.toLowerCase().replace(' ', '-');
	this.dataid1 = "dataVal-" + title.toLowerCase().replace(' ', '-')+'1';
	this.dataid2 = "dataVal-" + title.toLowerCase().replace(' ', '-')+'2';
...
RigStatusWidget.prototype = new Widget;

RigStatusWidget.prototype.init = function() {
    this.$widget = this.generateBox(this.id);
	this.enableDraggable();

};

RigStatusWidget.prototype.consume = function(data) {
	var msgToShow1;
	var msgToShow2;
	msgToShow1 = data[this.dataVar1];
	
    if (data[this.dataVar2]< 4){
		msgToShow2 = 'Initializing...Please wait...';
	}else if (data[this.dataVar2] ==4){
		msgToShow2 = 'Operating';
	}else{
		msgToShow2 = 'Clean up in progress...Please wait...';
	}
		
	document.getElementById(this.dataid1).innerHTML = msgToShow1;
	document.getElementById(this.dataid2).innerHTML = msgToShow2;
	
};


RigStatusWidget.prototype.getHTML = function() {
	

    var html = '<div class="place-holder place-holder-res">';
    html += '<p>' + this.message1 + '<span id="' + this.dataid1 + '" class="place-holder-message">-</span></p>';
	html += '<p>' + this.message2 + '<span id="' + this.dataid2 + '" class="place-holder-message">-</span></p>';
    html += '</div>';
    return html;
};
/* ============================================================================
 ...
 * ============================================================================ */
function PidCtrlWidget($container, title, icon, message, dataValPrefix, postActionPrefix)
{
	Widget.call(this, $container, title, icon);
	this.id = "Pid-" + title.toLowerCase().replace(' ', '-');
	this.dataValPrefix = dataValPrefix;
	this.postActionPrefix = postActionPrefix;
	...
	this.$opbox = undefined;
	this.$manBtn = undefined;
	this.$autBtn = undefined;
}
PidCtrlWidget.prototype = new Widget;

PidCtrlWidget.prototype.init = function() {
    this.$widget = this.generateBox(this.id);
	this.enableDraggable();
	this.enableResizable(200,335,false);
	var thiz = this;
	...
	this.$manBtn = $("#"+this.id+"-btn-man") 
		.click(function() { thiz.handleModeButtonClick(0); })
		.mousedown(function() { $(this).addClass("click-button-active");   })
		.mouseup(function() { $(this).removeClass("click-button-active") ; })
		.keypress(function(e) {
				if (e.keyCode == 13) thiz.handleModeButtonClick(0);
			});
	this.$autBtn = $("#"+this.id+"-btn-aut") 
		.click(function() { thiz.handleModeButtonClick(1); })
		.mousedown(function() { $(this).addClass("click-button-active");   })
		.mouseup(function() { $(this).removeClass("click-button-active") ; })
		.keypress(function(e) {
				if (e.keyCode == 13) thiz.handleModeButtonClick(1);
			});
	this.$spbox = $("#" + this.id + "-spVal")
		.focusin(formFocusIn)
		.focusout(formFocusOut)
		.change(function() { thiz.handleTextBoxChange($(this).val(),'SP'); });
	this.$opbox = $("#" + this.id + "-opVal")
		.focusin(formFocusIn)
		.focusout(formFocusOut)
		.change(function() { thiz.handleTextBoxChange($(this).val(),'OP'); });		
};

PidCtrlWidget.prototype.getHTML = function() {
	return(
		'<div id="PidCtrl-header" class="pid-header-div">' +
        	'<label for="pid-header-msg" class="pid-header-msg">'+ this.message + '<br>'+'Controller' + '</label>' +
		'</div>' +
		'<div class="pid-spacer-div">' +
			'<hr>'+
		'</div>'+
		'<div class="pid-body-div">' +
			'<label for="pid-PV" class="pid-label">PV:</label>'+
			'<span id="'+this.id+'-pvVal" class="pid-input">ABCD</span>'+
			'<label for="pid-PV-unit" class="pid-unit">degC</label>'+
		'</div>'+	
		'<div class="pid-body-div">' +
			'<label for="pid-SP" class="pid-label">SP:</label>'+
			'<input id="'+this.id+'-spVal" class="pid-input" type="text" tabindex="1"/>' +
			'<label for="pid-SP-unit" class="pid-unit">degC</label>'+
		'</div>'+	
		'<div class="pid-body-div">' +
			'<label for="pid-OP" class="pid-label">OP:</label>'+
			'<input id="'+this.id+'-opVal" class="pid-input" type="text" tabindex="2"/>' +
			'<label for="pid-OP-unit" class="pid-unit">%</label>'+
		'</div>'+	
		'<div class="pid-body-div">' +
			'<label for="pid-mode" class="pid-label">Mode:</label>'+
			'<span id="'+this.id+'-modeVal" class="pid-mode">abcd</span>'+
		'</div>'+
		'<div class="pid-spacer-div">' +
			'<hr>'+
		'</div>'+
		'<div class="pid-body-div">' +
			'<label for="mode-change" style="font-weight:bold">Mode Selection:</label>'+
		'</div>'+
		'<div class="pid-button-div">'+
			'<a id="'+this.id+'-btn-man" class="click-button click-button-disabled" tabindex="3" >MAN</a>' +
			'<a id="'+this.id+'-btn-aut" class="click-button click-button-disabled" tabindex="4" >AUTO</a>' +
		'</div>'
		
	);
}

PidCtrlWidget.prototype.consume = function(data) {
	var dataPV = this.dataValPrefix + 'PV';
	var dataSP = this.dataValPrefix + 'SP';
	var dataOP = this.dataValPrefix + 'OP';
	var dataMode = this.dataValPrefix + 'Mode';

	// Update values
	if (!(data[dataPV] == undefined || data[dataPV] == this.PV)){ 
		this.PV = data[dataPV];
		if (typeof(this.PV) != 'undefined' && this.PV != null) {
			document.getElementById(this.id+'-pvVal').innerHTML = this.PV.toPrecision(4);
		}
		else {
			document.getElementById(this.id+'-pvVal').innerHTML = 'undef';
		}
	}
	if (!(data[dataSP] == undefined || data[dataSP] == this.SP || this.spChanged)){
		this.SP = data[dataSP];
		if (typeof(this.SP) != 'undefined' && this.SP != null) {
			document.getElementById(this.id+'-spVal').value = this.SP.toPrecision(4);
		}
		else {
			document.getElementById(this.id+'-spVal').value = 'undef';
		}
	}
	if (!(data[dataOP] == undefined || data[dataOP] == this.OP || this.opChanged)){ 
		this.OP = data[dataOP];
		if (typeof(this.OP) != 'undefined' && this.OP != null) {
			document.getElementById(this.id+'-opVal').value = this.OP.toPrecision(4);
		}
		else {
			document.getElementById(this.id+'-opVal').value = 'undef';
		}
	}
	if (data[dataMode] == 0){this.Mode = 'MANUAL';}
	if (data[dataMode] == 1){this.Mode = 'AUTO';}
		if (typeof(this.Mode) != 'undefined' && this.Mode != null) {
			document.getElementById(this.id+'-modeVal').innerHTML = this.Mode;
		}
		else {
			document.getElementById(this.id+'-modeVal').innerHTML = 'undef';
		}
	// interlocking
	if (data[dataMode] == 0){
		document.getElementById(this.id+'-spVal').disabled = true;
		document.getElementById(this.id+'-opVal').disabled = false;
		this.$manBtn.addClass("click-button-disabled");
		this.$autBtn.removeClass("click-button-disabled");
	}	
	if (data[dataMode] == 1){
		document.getElementById(this.id+'-spVal').disabled = false;
		document.getElementById(this.id+'-opVal').disabled = true;
		this.$manBtn.removeClass("click-button-disabled");
		this.$autBtn.addClass("click-button-disabled");
	}
};

PidCtrlWidget.prototype.handleTextBoxChange = function(val,type) {
	
	
    var ttLeft = 164,
		ttTopSP = 155;
        ttTopOP  = 179;
    if (type == 'SP'){ ttTop = ttTopSP;}
	if (type == 'OP'){ ttTop = ttTopOP;}
	...
	if ((((n > 60)||(n < 0))&&(type == 'SP'))||(((n > 100)||(n < 0))&&(type == 'OP'))){
	...
	this.sendValue(n,type);	
};

PidCtrlWidget.prototype.sendValue = function(val,type){
	var thiz = this, params = { };
	var newParam = this.postActionPrefix.toLowerCase() + type +'target';
	var newAction = 'set' + this.postActionPrefix + type;
    params[newParam] = val;
    this.postControl(newAction, params,
        function(data) {
			if (type == 'SP'){ thiz.spChanged = false;}
			if (type == 'OP'){ thiz.opChanged = false;}
		}
    );
}
PidCtrlWidget.prototype.handleModeButtonClick = function(mode){
	var thiz = this, params = {}; 
	var newParam = this.postActionPrefix.toLowerCase() + 'ModeTarget';
	var newAction = 'set' + this.postActionPrefix + 'Mode';
	params[newParam] = mode;
	this.postControl(newAction, params,
        function(data) {}
    );
}
 ...
 * == PID Tuning Widget		                                             ==
 * ============================================================================ */
function PidTuneWidget($container, title, icon, message, dataValPrefix, postActionPrefix)
{
	Widget.call(this, $container, title, icon);
	this.id = "Pid-" + title.toLowerCase().replace(' ', '-');
	this.dataValPrefix = dataValPrefix;
	this.postActionPrefix = postActionPrefix;
	this.kpChanged = false;
	this.kiChanged = false;
	this.kdChanged = false;
	...
	this.$kpbox = undefined;
	this.$kibox = undefined;
	this.$kdbox = undefined;

}
PidTuneWidget.prototype = new Widget;

PidTuneWidget.prototype.init = function() {
    this.$widget = this.generateBox(this.id);
	this.enableDraggable();
	this.enableResizable(220,230,false);
	var thiz = this;
	...
	this.$kpbox = $("#" + this.id + "-kpVal")
		.focusin(formFocusIn)
		.focusout(formFocusOut)
		.change(function() { thiz.handleTextBoxChange($(this).val(),'KP'); });
	this.$kibox = $("#" + this.id + "-kiVal")
		.focusin(formFocusIn)
		.focusout(formFocusOut)
		.change(function() { thiz.handleTextBoxChange($(this).val(),'KI'); });	
	this.$kpbox = $("#" + this.id + "-kdVal")
		.focusin(formFocusIn)
		.focusout(formFocusOut)
		.change(function() { thiz.handleTextBoxChange($(this).val(),'KD'); });		
};

PidTuneWidget.prototype.getHTML = function() {
	return(
		'<div id="PidCtrl-header" class="pid-header-div">' +
        	'<label for="pid-header-msg" class="pid-header-msg">'+ this.message + '<br>'+'Tuning' + '</label>' +
		'</div>' +
		'<div class="pid-spacer-div">' +
			'<hr>'+
		'</div>'+
		'<div class="pid-body-div">' +
			'<label for="pid-KP" class="pid-label">Gain:</label>'+
			'<input id="'+this.id+'-kpVal" class="pid-input" type="text" tabindex="1"/>' +
		'</div>'+	
		'<div class="pid-body-div">' +
			'<label for="pid-KI" class="pid-label">T1:</label>'+
			'<input id="'+this.id+'-kiVal" class="pid-input" type="text" tabindex="2"/>' +
		'</div>'+	
		'<div class="pid-body-div">' +
			'<label for="pid-KD" class="pid-label">T2:</label>'+
			'<input id="'+this.id+'-kdVal" class="pid-input" type="text" tabindex="3"/>' +
		'</div>'+	
		'<div class="pid-spacer-div">' +
			'<hr>'+
		'</div>'
...
PidTuneWidget.prototype.consume = function(data) {
	var dataKP = this.dataValPrefix + 'KP';
	var dataKI = this.dataValPrefix + 'KI';
	var dataKD = this.dataValPrefix + 'KD';

	// Update values
	if (!(data[dataKP] == undefined || data[dataKP] == this.KP)){ 
		this.KP = data[dataKP];
		if (typeof(this.KP) != 'undefined' && this.KP != null) {
			document.getElementById(this.id+'-kpVal').value = this.KP.toPrecision();
		}
		else {
			document.getElementById(this.id+'-kpVal').value = 'undef';
		}
	}
	if (!(data[dataKI] == undefined || data[dataKI] == this.KI)){ 
		this.KI = data[dataKI];
		if (typeof(this.KI) != 'undefined' && this.KI != null) {
			document.getElementById(this.id+'-kiVal').value = this.KI.toPrecision();
		}
		else {
			document.getElementById(this.id+'-kiVal').value = 'undef';
		}
	}
	if (!(data[dataKD] == undefined || data[dataKD] == this.KD)){ 
		this.KD = data[dataKD];
		if (typeof(this.KD) != 'undefined' && this.KD != null) {
			document.getElementById(this.id+'-kdVal').value = this.KD.toPrecision();
		}
		else {
			document.getElementById(this.id+'-kdVal').value = 'undef';
		}
	}
};

PidTuneWidget.prototype.handleTextBoxChange = function(val,type) {
    ...
    var ttLeft = 164,
		...
		ttTopKI = 155;
        ttTopKD  = 179;
    if (type == 'KP'){ ttTop = ttTopKP;}
	if (type == 'KI'){ ttTop = ttTopKI;}
	if (type == 'KD'){ ttTop = ttTopKD;}
...
PidTuneWidget.prototype.sendValue = function(val,type){
	var thiz = this, params = { };
	var newParam = this.postActionPrefix.toLowerCase() + type +'target';
	var newAction = 'set' + this.postActionPrefix + type;
    params[newParam] = val;
    this.postControl(newAction, params,
        function(data) {
			if (type == 'KP'){ thiz.kpChanged = false;}
			if (type == 'KI'){ thiz.kiChanged = false;}
			if (type == 'KD'){ thiz.kdChanged = false;}
		}
    );
}
...
Widget.prototype.postControl = function(action, params, responseCallback, errorCallback) {
    var thiz = this;
	$.ajax({
        url: "/primitive/mapjson/pc/BarTemperatureRigController/pa/" + action,
        data: params,
        success: function(data) {
...
CameraWidget.prototype.consume = function(data) {
	...
	*/
	...
	ctx = document.getElementById('videoDrawingCanvas').getContext('2d');
	ctx.save();
   ...
   var temp;
	var ValToShow,ValToCompare;
	var j,k,colourB,colourR = 0;
	var num = "00B0";
	var uni = '"\\u' + num+'"';
	var hexstring = eval(uni);
 	for(var i = 1; i <= 11; i++){
		if (data["CurTemp"+i]=='Bad'){
			ValtoShow = 'Bad';
			}
		else{
			ValToShow =(data["CurTemp"+i]).toPrecision(3);
			colourB = Math.floor((ValToShow<40 ? 255 : (ValToShow > 200 ? 0 : (200-ValToShow)*255/100)));
			colourR = Math.floor((ValToShow<40 ? 0 : (ValToShow> 200 ? 255 : (ValToShow)*255/100)));
			}
		ctx.fillStyle = "rgba(" + colourR + ", 0, " + colourB + ", 0.45)";
		ctx.fillRect(this.videoWidth*(1/10*(i-1)), 0, this.videoWidth*(1/10), this.videoHeight);
		...
		ctx.fillStyle="#ffffff";
			if((i%2)==0){
				...
				j=this.videoHeight*0.3+10;
				k=this.videoHeight*0.4+10;
				}
		ctx.font = "700 20px Georgia";
	
		ctx.fillText(i, this.videoWidth*(0.05+0.9/10*(i-1))-10,this.videoHeight*0.35+j);
		ctx.font = "700 10px Georgia";	
		
		ctx.fillText( ValToShow+hexstring+"C", this.videoWidth*(0.05+0.9/10*(i-1))-10,this.videoHeight*0.3+k);
		
		
	}	 
	// var colourB = Math.floor((arrow2<0 ? 255 : (arrow2 > 100 ? 0 : (100-arrow2)*255/100)));
	// var colourR = Math.floor((arrow2<0 ? 0 : (arrow2 > 100 ? 255 : (arrow2)*255/100)));
	// ctx.fillStyle = "rgba(" + colourR + ", 0, " + colourB + ", 0.3)";
	// ctx.fillRect(0, 0, this.videoWidth, this.videoHeight);


	// ctx.clearRect(this.videoWidth*0.25,this.videoHeight*0.35,this.videoWidth*0.5,this.videoHeight*0.3);
	// ctx.clearRect(0,this.videoHeight*0.35,this.videoWidth,this.videoHeight*0.3);
	...
	// PRETEND BAR
		ctx.fillStyle = "#ffffff";
		ctx.fillRect(this.videoWidth*0.05,this.videoHeight*0.38,this.videoWidth*0.9,this.videoHeight*0.24);
		
		ctx.strokeStyle="rgba(0,255,0,1)";
		ctx.fillStyle = "#00ff00";
		ctx.fillRect(0,this.videoHeight*0.38,this.videoWidth*0.05,this.videoHeight*0.24);
	...
	/* SMALL ARROW SHOWING GRADIENT COMPARED TO LAST ONLY*/
	/*	
	 	for(var i = 1; i <= 11; i++){
			if (data["CurTemp"+i]!='Bad'){
				ValToShow =( (data["CurTemp"+(i)])).toPrecision(3);
				if (i==1){
					ValToCompare= ( (data["HeaterPV"])).toPrecision(3);
				}
				else{
					ValToCompare= ( (data["CurTemp"+(i-1)])).toPrecision(3);
				}
				gradient = ValToShow/ValToCompare;
				drawArrow(ctx, this.videoWidth*(0.05+0.9/10*(i-1)), this.videoHeight*0.5, this.videoWidth*(0.05+0.9/10*(i)), this.videoHeight*0.5*gradient, arrowStyle, arrowPos, arrowAngle, arrowSize);
				drawArrow(ctx, this.videoWidth*(0.05+0.9/10*(i-1)), this.videoHeight*0.35, this.videoWidth*(0.05+0.9/10*(i-1)), this.videoHeight*0.35+50*(gradient-1), arrowStyle, arrowPos, arrowAngle, arrowSize);
			}	
		
		} 
		
	*/	
		/*FGRAPH COMPARING EACH SECTION TO OVERALL TEMP CHANE*/
	/*	
	ctx.beginPath();
	ctx.moveTo(0,this.videoHeight*(200-data["HeaterPV"])/160);
	temp=(200-data["HeaterPV"])/160;
	for(var i = 1; i <= 11; i++){
		if (data["CurTemp"+i]!='Bad'){
			 ValToShow =( (data["CurTemp"+(i)])).toPrecision(3);	
			 gradient = (200-ValToShow)/160;
			
				ctx.lineTo(this.videoWidth*(0.05+0.9/10*(i)),this.videoHeight*gradient);
				//drawArrow(ctx, this.videoWidth*(0.05+0.9/10*(i-1)), this.videoHeight*0.5, this.videoWidth*(0.05+0.9/10*(i)), this.videoHeight*0.5*gradient, arrowStyle, arrowPos, arrowAngle, arrowSize);
				drawArrow(ctx, this.videoWidth*(0.05+0.9/10*(i-1)), this.videoHeight*temp, this.videoWidth*(0.05+0.9/10*(i)), this.videoHeight*gradient, arrowStyle, arrowPos, arrowAngle, arrowSize);
	...
	ctx.stroke();
	*/	
		// /*GRAPH COMPARING EACH SECTION TO OVERALL TEMP CHANE only inside box*/
				// ctx.strokeStyle="rgba(0,0,0,1)";
	/*			
	ctx.beginPath();
	ctx.moveTo(0,this.videoHeight*(0.38+0.24*(200-data["heaterTemp"])/160));
	for(var i = 1; i <= 11; i++){
		if (data["CurTemp"+i]!='Bad'){
			ValToShow =( (data["CurTemp"+(i)])).toPrecision(3);	
			gradient = (200-ValToShow)/160*0.24;
			ctx.lineTo(this.videoWidth*(0.05+0.9/10*(i)),this.videoHeight*(0.38+gradient));
		}
	}		
	ctx.stroke();
	...
	//ctx.strokeRect(this.videoWidth*0.05,this.videoHeight*0.38,this.videoWidth*0.9,this.videoHeight*0.24);
	
		//var drawArrow=function(ctx,x1,y1,x2,y2,style,which,angle,d)	
	//drawArrow(ctx, 50, 100, 100+arrowLength,50-arrowLength, arrowStyle, arrowPos, arrowAngle, arrowSize);
	*/
	
/* 
	// DRAWS THE ARROWs
	for(var i =0; i<this.videoWidth; i=i+10){
		//drawArrow(ctx, 0, i, arrow2, i, arrowStyle, arrowPos, arrowAngle, arrowSize);
		if (this.videoHeight*0.35+i>=this.videoHeight*0.65){ break; }	
		// if (this.videoWidth*0.25+arrow2/4>=this.videoWidth*0.75){ arrow2=4*this.videoWidth*0.5;}
		if (arrow2/4>=this.videoWidth){ arrow2=this.videoWidth;}
		// drawArrow(ctx, this.videoWidth*0.25, this.videoHeight*0.35+i, this.videoWidth*0.25+arrow2/4, this.videoHeight*0.35+i, arrowStyle, arrowPos, arrowAngle, arrowSize);
		drawArrow(ctx, 0, this.videoHeight*0.35+i, arrow2/4, this.videoHeight*0.35+i, arrowStyle, arrowPos, arrowAngle, arrowSize);
		}
    ...
    //var drawArrow=function(ctx,x1,y1,x2,y2,style,which,angle,d)	
	// ctx.strokeStyle="rgba(128,255,128,1)";
	// ctx.beginPath();
	// ctx.moveTo(80,80);
	// ctx.lineTo(120,120);
	// ctx.lineTo(120,180);
	...
	// ctx.arc(120,180,40,0,1.5*Math.PI,true);
	// ctx.stroke();
	// ctx.fillStyle="rgba(128,255,128,1)";
	// drawArrow(ctx, 120,140,119,140,arrowStyle, arrowPos, arrowAngle, arrowSize);
...
CameraWidget.prototype.deploy = function(format) {
	...
	//This sets the video-player div to have the camera background
	this.$widget.find(".video-player").css("width", this.videoWidth);
	this.$widget.find(".video-player").css("height", this.videoHeight);
	this.$widget.find(".video-player").css("background", "url(" + 
												this.urls.mjpeg + "?" + new Date().getTime() + ")");
	this.$widget.find(".video-player").css("background-size", "100% 100%");
	
	//Arrow
    this.$widget.find(".video-player").html("<canvas id='videoDrawingCanvas' width='" + this.videoWidth + "' height='" + this.videoHeight + "' > </canvas>");
...
function DataValWidget($container, title, icon, message, dataVar)
{
	...
	//this.colour ='colour';
	this.colourid= "dataVal2-"+ title.toLowerCase().replace(' ', '-');
	this.i='i';
}
...
DataValWidget.prototype.consume = function(data) {
	...
	else if(this.dataVar == 'counter'){
		if ( valToShow < 100){
			colour = 'blue';
		}
		else if (valToShow > 200){
			colour = 'red';
		}
		else{
			colour = 'black';
	...
	// background-image:linear-gradient(to bottom, rgba(240, 255, 40, 1) 0%, rgba(240, 255, 40, 1) 100%)
	document.getElementById(this.dataid).innerHTML = '<span style="color: '+colour+';">'+valToShow.toPrecision(6)+'</span>';
 ...
 * Information displaying widget.
 */
function DataValWidgetTwo($container, title, icon, message1, message2, dataVar1, dataVar2)
{
    Widget.call(this, $container, title, icon);
    ...
    this.id = "DataVal-" + title.toLowerCase().replace(' ', '-');
	this.dataid = "dataVal-" + title.toLowerCase().replace(' ', '-');
    // this.id2 = "DataVal-2-" + title.toLowerCase().replace(' ', '-');
	this.dataid2 = "dataVal-2-" + title.toLowerCase().replace(' ', '-');	
    /** Message that is displayed by this widget. */
...
DataValWidgetTwo.prototype = new Widget;

DataValWidgetTwo.prototype.init = function() {
    this.$widget = this.generateBox(this.id);
	this.enableDraggable();
};

DataValWidgetTwo.prototype.consume = function(data) {
    valToShow1 = data[this.dataVar1];
    valToShow2 = data[this.dataVar2];
	document.getElementById(this.dataid).innerHTML = valToShow1.toPrecision(6);
	document.getElementById(this.dataid2).innerHTML = valToShow2.toPrecision(6);	
	var colour1 = 'orange';
	if (this.dataVar1 == 'increment'){
		if ( valToShow1 < -5){
			colour1 = 'blue';
		}
		else if (valToShow1 > 5){
			colour1 = 'red';
		}
		else{
			colour1 = 'black';	
	...
	else if(this.dataVar1 == 'counter'){
		if ( valToShow1 < 100){
			colour1 = 'blue';
		}
		else if (valToShow1 > 200){
			colour1 = 'red';
		}
		else{
			colour1 = 'black';
		...
		var colour2 = 'orange';
	if (this.dataVar2 == 'increment'){
		if ( valToShow2 < -5){
			colour2 = 'blue';
		}
		else if (valToShow2 > 5){
			colour2 = 'red';
		}
		else{
			colour2 = 'black';	
	...
	else if(this.dataVar2 == 'counter'){
		if ( valToShow2 < 100){
			colour2 = 'blue';
		}
		else if (valToShow2 > 200){
			colour2 = 'red';
		}
		else{
			colour2 = 'black';
	...
	// background-image:linear-gradient(to bottom, rgba(240, 255, 40, 1) 0%, rgba(240, 255, 40, 1) 100%)
	document.getElementById(this.dataid).innerHTML = '<span style="color:white;background-color: '+colour1+';">'+valToShow1.toPrecision(6)+'</span>';
	// background-image:linear-gradient(to bottom, rgba(240, 255, 40, 1) 0%, rgba(240, 255, 40, 1) 100%)
	document.getElementById(this.dataid2).innerHTML = '<span style="color:white;background-color: '+colour2+';">'+valToShow2.toPrecision(6)+'</span>';
};


DataValWidgetTwo.prototype.getHTML = function() {
    var html = '<div class="place-holder place-holder-res">';
	html += '<p>' + this.message1 + '<span id="' + this.dataid + '" class="place-holder-message">-</span></p>';
	html += '<p>' + this.message2 + '<span id="' + this.dataid2 + '" class="place-holder-message">-</span></p>';
    html += '</div>';
    return html;
};
...
/* ============================================================================
 * == Data Val Widget            Sensor                                         ==
 * ============================================================================ */

/**
 * Information displaying widget .
 */
function DataValWidgetSensor($container, title, icon, dataVar, msg)
{
    Widget.call(this, $container, title, icon);
	...
	this.id= "DataVal-" + title.toLowerCase().replace(' ', '-');
	for (var i  = 0; i < 11; i++){
		dataids[i] = "dataVal-" + title.toLowerCase().replace(' ', '-')+i;
	}
    this.dataid=dataids;
    /** Message that is displayed by this widget. */
    this.message = msg;
...
DataValWidgetSensor.prototype = new Widget;

DataValWidgetSensor.prototype.init = function() {
    this.$widget = this.generateBox(this.id);
	this.enableDraggable();
...
DataValWidgetSensor.prototype.consume = function(data) {
	...
	//var temp = 1;
	// var temp = data["heaterTemp"];
	for(var i = 0; i < 11; i++){
		if (data[this.dataVar+(i+1)] == 'Bad'){
			colour = "rgb(0,0,0)";
			document.getElementById(this.dataid[i]).innerHTML = '<span style="color: '+colour+';"> Bad </span>';
		}
		else{
			//ValToShow = (data[this.dataVar+i])/temp;
			ValToShow = (data[this.dataVar+(i+1)]);
			colourB = Math.floor((ValToShow<40 ? 255 : (ValToShow > 200 ? 0 : (100-ValToShow)*255/100)));
			colourR = Math.floor((ValToShow<40 ? 0 : (ValToShow> 200 ? 255 : (ValToShow)*255/100)));
			colour = "rgb("+colourR+",0,"+colourB+")";
			document.getElementById(this.dataid[i]).innerHTML = '<span style="color: '+colour+';">'+ValToShow.toPrecision(6)+'</span>';
			//temp=temp*20;
		}
	}
};
...
DataValWidgetSensor.prototype.getHTML = function() {
	

    var html = '<div class="place-holder place-holder-res">';
	for ( var i = 0; i < 11; i++){
		html += '<p>' + this.message + (i+1) + '= <span id="' + this.dataid[i] + '" class="place-holder-message">-</span></p>';
	}
    html += '</div>';
    return html;
};
...
GraphWidget.prototype.acquireData = function() {
        var thiz = this;
        $.ajax({
                url: "/primitive/mapjson/pc/" + this.controller + "Controller/pa/graphData",
                data: {
                        period: this.period,
